...
#
# Everything below is computed from real rows in the DB for a fixed 30-day
# window (the app no longer exposes a range selector, so this is the single
# hard ceiling used everywhere), split into an "earlier half" and a "later
# half" so we can measure a genuine trend rather than a single snapshot. The
# headline/subtext copy is then generated by an LLM grounded strictly in
# that computed `changes` data (never given raw DB access, never allowed to
# invent a number) — with a deterministic template fallback if the LLM call
# fails or returns something unparseable, so this endpoint is never blank or
# broken.
#
...
# Hard ceiling — the app has no range selector anymore, every insights
# request analyzes at most this many trailing days of data. If a user has
# more history than this, older rows are never even fetched (see the
# `.gte(date_col, cutoff)` filters below), let alone sent to the LLM.
MAX_INSIGHT_DAYS = 30
...
def _earliest_date_str(rows: list, date_field: str) -> str | None:
    """Return the earliest date/measured_at value (as a string) in rows, or
    None if rows is empty. Works for both plain 'date' columns (YYYY-MM-DD)
    and 'measured_at' ISO timestamp columns since we only need string min()
...
async def get_insights(user_id: str):
    ...
    try:
        ...
        # --- Actual days of data used (for the frontend's "YOUR LAST N DAYS" label) ---
        # Collect the earliest timestamp seen across every metric queried this
        # request, then compute the real calendar-day span from that to now,
        # capped at MAX_INSIGHT_DAYS. This reflects true data coverage — e.g.
        # a user who only has 6 days of real readings sees "6 DAYS", not a
        # hardcoded "30 DAYS" that overstates what was actually analyzed. If
        # a user has more than MAX_INSIGHT_DAYS of history, this still caps
        # at MAX_INSIGHT_DAYS since older rows were never fetched to begin
        # with (see the `.gte(date_col, cutoff)` filters above).
        earliest_candidates = [
            _earliest_date_str(sleep_rows, "date"),
            _earliest_date_str(hrv_rows, "date"),
            _earliest_date_str(hr_rows, "date"),
            _earliest_date_str(bp_rows, "measured_at"),
            _earliest_date_str(spo2_rows, "date"),
            _earliest_date_str(steps_rows, "date"),
            _earliest_date_str(stress_rows, "measured_at"),
            _earliest_date_str(temp_rows, "measured_at"),
        ]
        earliest_candidates = [c for c in earliest_candidates if c]

        if earliest_candidates:
            earliest_str = min(earliest_candidates)
            try:
                if "T" in earliest_str or " " in earliest_str:
                    earliest_dt = datetime.fromisoformat(earliest_str.replace("Z", "+00:00"))
                else:
                    earliest_dt = datetime.fromisoformat(earliest_str).replace(tzinfo=timezone.utc)
                span_days = (datetime.now(timezone.utc) - earliest_dt).days + 1
                days_used = max(1, min(span_days, days))
            except Exception:
                days_used = days
        else:
            days_used = 0  # no data at all for this user in the window
